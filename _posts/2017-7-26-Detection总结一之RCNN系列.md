...
同时此处需要注意的是, 经过Spatial Pyramid Pooling的feature先保存在了硬盘了, 所以在用Softmax进行finetune的时候只对剩下的几个全连接层进行了fine-tune. 这个问题将在Fast RCNN里得到解决. 其之所以不能finetune后面卷积层的原因我们在后面说.    
   
![sppnet2](/images/0726/sppnet2.png)  
   
**Spatial Pyramid Pooling基本含义**  
我们知道传统的pooling操作是以固定的窗口大小对feature map进行滑动得到, 得到的输出与输入大小有关. 而Spatial Pyramid Pooling解决统一输出的关键在于, 使用的是输入feature map的*相对大小*来进行滑动, 这样便能针对不同大小的feature map有不同大小的窗口, 从而得到统一大小的输出了.    
      
同时为了提取多尺度信息, 进行几个不同scale的相对大小pooling操作, 将它们拼接起来, 便形成了Spatial Pyramid Pooling的输出结果.    
...
比如在Fast R-CNN中, 先随机挑选了N张图像, 进行前向传播, 然后再从每张图像中选择\\(\frac{R}{N}\\)个Region proposal进行计算和更新. 但是在SPPNet中, 如果要达到R个Region proposal同样的效果便需要前向传播R张图像(假设R个区域来自不同的图像). 显然Fast R-CNN要高效得多, 同时由于前面的卷积层得到了更新, 整体检测效果也得到了提升.     
...
在Fast R-CNN后这一体系的相关方法便只剩Region Proposal的提取为速度和精度瓶颈了. Faster R-CNN提出了Region Proposal Networks(RPN)用于提取proposal, 通过与Fast R-CNN共享大部分卷积层, 实现了实时的Region Proposal的提取, 同时由于这些proposal从数据中学得, 会收益于好的feature.  Faster R-CNN可以看做是RPN与Fast R-CNN的一个结合.    
...
[He K, Zhang X, Ren S, et al. Spatial pyramid pooling in deep convolutional networks for visual recognition[C]//European Conference on Computer Vision. Springer, Cham, 2014: 346-361.](https://link.springer.com/chapter/10.1007/978-3-319-10578-9_23)  
[Girshick R. Fast r-cnn[C]//Proceedings of the IEEE international conference on computer vision. 2015: 1440-1448.](http://www.cv-foundation.org/openaccess/content_iccv_2015/html/Girshick_Fast_R-CNN_ICCV_2015_paper.html)   
[Ren S, He K, Girshick R, et al. Faster R-CNN: Towards real-time object detection with region proposal networks[C]//Advances in neural information processing systems. 2015: 91-99.](http://papers.nips.cc/paper/5638-faster-r-cnn-towards-real-time-object-detection-with-region-proposal-networks)  
[Zhao H, Shi J, Qi X, et al. Pyramid scene parsing network[J]. arXiv preprint arXiv:1612.01105, 2016.](https://arxiv.org/abs/1612.01105)   
